...
# [추가] 대시보드 통계 구역 (발표용 핵심 비주얼)
st.write("### 📊 오늘의 거지방 동향")
total_goods = sum([p['good'] for p in st.session_state.posts])
total_bads = sum([p['bad'] for p in st.session_state.posts])

col_stat1, col_stat2 = st.columns(2)
with col_stat1:
    st.markdown(f"<div class='stat-card' style='color:#2b8a3e;'>👏 오늘 참아낸 횟수: {total_goods}회</div>", unsafe_allow_html=True)
with col_stat2:
    st.markdown(f"<div class='stat-card' style='color:#c92a2a;'>🚨 대가리 깨진 횟수: {total_bads}회</div>", unsafe_allow_html=True)

# 차트 데이터 시각화
chart_data = pd.DataFrame({
    '항목': [p['item'] for p in st.session_state.posts],
    '때찌(혼난 횟수)': [p['bad'] for p in st.session_state.posts],
    '참음(칭찬 횟수)': [p['good'] for p in st.session_state.posts]
}).set_index('항목')
st.bar_chart(chart_data, color=["#ff8787", "#74c0fc"])
    ...
    for idx, post in enumerate(st.session_state.posts):
        ...
        with col_b:
            if st.button(f"🔴 때찌 정신차려 ({post['bad']})", key=f"bad_{post['id']}_{idx}"):
                post['bad'] += 1
                # [추가] 누를 때마다 무작위 잔소리가 튀어나오는 다이내믹 팝업 효과
...
with tab3:
    ...
    if st.button("💸 운명의 동전 던지기"):
        if gamble_item:
            with st.spinner("거지방 신령님이 주사위를 굴리는 중..."):
                time.sleep(1) # 극적인 긴장감을 주는 연출 딜레이
            
            result = random.choice(["앞면", "뒷면"])
            if result == "앞면":
                st.error(f"❌ 결과: [앞면] -> 절대 사지 마세요! '{gamble_item}' 사면 이번 달 내내 물만 마셔야 합니다.")
            else:
                st.warning(f"⚠️ 결과: [뒷면] -> 사지 마세요! 뒤를 돌아보지 말고 도망치세요.")
        else:
